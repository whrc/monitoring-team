...
yk2nee = ggplot(data = test.data.nee,aes(rfnee,co2_flux.c))+theme_bw()+
  ...
  annotate(geom = 'text',x = 6, y = -8,label=expression(R^2~"= 0.75"),size = 3)+
...
yk2ch4 = ggplot(data = test.data.ch4,aes(rfch4,ch4_flux.c))+theme_bw()+
  ...
  annotate(geom= 'text',x = 0.07,y = -0.035,label = expression(Slope~"= 1.08"),size = 3)+
  theme(text = element_text(size = 8))
...
png(filename = 'C:/Users/karndt.WHRC/Desktop/sites/YKD/plots/yk2unburned gapfilling validation.png',width = 6,height = 4,units = 'in',res = 1000)
