...
knitr::opts_knit$set(root.dir = 'C:/Users/karndt.WHRC/Desktop/sites/YKD/data/unburned/') #use this to set your directory
```
...
df$TA_1_1_1  = na.spline(df$TA_1_1_1-273.15,maxgap = gap)
...
```{r}
write.csv(x = df,file = "./ykd_unburned_2019_2022.csv",row.names = F)
```
...
ggplot(data = all)+
  ...
  geom_point(aes(date,all$TA_1_1_1,col='Site'))


airt.mod = lm(formula = all$TA_1_1_1 ~ all$airt)
summary(airt.mod)

all$airt.eramod = all$airt*airt.mod$coefficients[2] + airt.mod$coefficients[1]     

ggplot(data = all,aes(airt,TA_1_1_1))+theme_bw()+
  geom_point()+
  geom_smooth(method = 'lm')+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)
```
...
write.csv(x = df,file = './ykd_unburned_2019_2022_era.csv',row.names = F,quote = F)
```
