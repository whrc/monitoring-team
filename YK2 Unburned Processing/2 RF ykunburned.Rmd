...
Load in data
```{r,error=FALSE,warning=FALSE}
cf = fread('./ykd_unburned_2019_2024_era.csv')
...
ggplot(data = cf)+theme_bw()+
  geom_point(aes(date,rfnee,col='RF'),alpha=0.5)+
  geom_point(aes(date,co2_flux.c,col='Real'),alpha=0.5)+
  scale_x_datetime(limits = as.POSIXct(c("2024-06-15","2024-06-30")))
...
save off the data
```{r,error=FALSE,warning=FALSE}
write.csv(x = cf,file = "./ykd_unburned_2019_2024_gf.csv",quote = F,row.names = F)
```
