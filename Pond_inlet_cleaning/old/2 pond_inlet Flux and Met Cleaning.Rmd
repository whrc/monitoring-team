...
met = fread('C:/Users/klynoe/Documents/pond_inlet/R_outputs/pondinlet_met_clean_202411.csv')

df = fread('C:/Users/klynoe/Documents/pond_inlet/R_outputs/pond_inlet_fluxes_merged_2024_07_11.csv')
...
# Filter rows with TIMESTAMP greater than "2024-07-26 16:30:00"
filter_date <- as.POSIXct("2024-07-26 16:30:00", tz = "UTC")
#filter_date <- as.POSIXct("2024-10-30 16:30:00", tz = "UTC")
#df <- df[!(df$TIMESTAMP <= filter_date), ]
#df <- df[!is.na(df$TIMESTAMP), ]

met <- met[!(met$TIMESTAMP <= filter_date), ]
met <- met[!is.na(met$TIMESTAMP), ]
...
cleaning code. Used to easily change date range for different data sets. 
```{r, warning=FALSE}
flux_dates <- c("2024-07-26",'2024-12-01')
```
# Initial Clean
...
```{r, warning=FALSE}
co2_flux.limits = replace(df$co2_flux,(df$co2_flux > 7 | df$co2_flux < -10) & df$TIMESTAMP >= '2024-07-26' | (df$co2_flux > 10 | df$co2_flux < -14),NA)
...
diel = df %>%
  group_by(hour,month) %>%
  select(co2_flux.c,LE.c,H.c,hour,month) %>%
...
write.csv(x = df,file = 'C:/Users/klynoe/Documents/pond_inlet/R_outputs/pond_inlet_Flux_Met_Clean_Aug_Nov_24.csv',row.names = F,quote = F)
```

#NEST AND SPLIT THE DATA BY MONTH AND SAVE IN DESIRED DIRECTORY
```{r, warning=FALSE}

base_path = "C:/Users/klynoe/Documents/pond_inlet/R_outputs/met_and_flux/"
