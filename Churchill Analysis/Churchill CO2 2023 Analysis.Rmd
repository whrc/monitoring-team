...
df_2023 <- df_2023 %>%
  mutate(FC_night = ifelse(NETRAD <= 0, FC, NA))
...
ggplot(data = df_avg_2023, aes(x=DOY, y=cumulative, color = season))+
  ...
  geom_hline(yintercept=0, col="black")+ 
  scale_y_continuous(expression('Cumulative NEE ('*m*mol~m^-2*')'))
...
# FC_night used to estimate RECO without temp correlation
ggplot(data=df_2023)+
  geom_point(aes(x=TS_1_05_1, y=FC_night, col = season))+
  labs(
    x = "Soil Temp",
