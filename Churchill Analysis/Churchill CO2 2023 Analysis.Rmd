...
ggplot(data = df_2023, aes(x=TIMESTAMP))+
  theme_bw()+
  geom_point(aes(y = -GPP_F, color = "GPP")) +
  geom_point(aes(y = RECO, color = "Respiration")) +
  geom_hline(yintercept=0, col="black")+
  scale_y_continuous(expression('Half-Hourly CO'[2]*' Flux ('*m*mol~m^-2~s^-1*')'))+
  scale_x_datetime(expression('Date'))+
  scale_color_manual(name = " ", values=c("turquoise3", "salmon2"))
...
## change to ameriflux submission to get different value? 11/12 ish?
...
#change units to grams/day

ggplot(data = df_avg_2023) +
  theme_bw() +
  geom_hline(yintercept=0, colour = "black")+
  geom_line(aes(x = day, y = TS_1_0_1/15, colour = "10cm Soil Temperature")) +
  geom_line(aes(x = day, y = TA/15, colour = "Air Temperature")) +
  geom_point(aes(x = day, y = FC_F*60*60*24*(1/1000000)*12, colour = "Net CO2 Flux"))+
  scale_y_continuous(name = expression("NEE (gC"~m^-2*~d^-1)"), sec.axis = sec_axis(~ . * 15, name = "Temperature (C)"))+
  scale_x_datetime(name="Date")+
  scale_color_manual(
    name = "Plots",
    values = c("10cm Soil Temperature" = "darkblue", "Air Temperature" = "salmon", "Net CO2 Flux" = "turquoise3"))+
  geom_vline(xintercept = as.POSIXct("2023-05-10"))+ #end of winter, DOY 130
  ...
  geom_vline(xintercept =  as.POSIXct("2023-10-27"))
  
  #end of zero curtain, DOY 300



# This is the same plot, just without a legend so it has better proportions

ggplot(data = df_avg_2023) +
  theme_bw() +
  geom_hline(yintercept=0, colour = "black")+
  geom_line(aes(x = day, y = TS_1_0_1/15), colour = "darkblue") +
  geom_line(aes(x = day, y = TA/15), colour = "salmon") +
  geom_point(aes(x = day, y = FC_F), colour = "turquoise3")+
  scale_y_continuous(name = expression(" CO"[2] * " Flux (Micromoles of CO"[2] * "/ m"^2*"s)"), sec.axis = sec_axis(~ . * 15, name = "Temperature (C)"))+
  scale_x_datetime(name="Date")+
  geom_vline(xintercept = as.POSIXct("2023-05-10"))+ #end of winter, DOY 130
  geom_vline(xintercept =  as.POSIXct("2023-06-12"))+ #end of snowmelt, DOY 163
  geom_vline(xintercept =  as.POSIXct("2023-09-06"))+ #end of growing season, DOY 249
...
seasonal_integrals_2023$season <- factor(seasonal_integrals_2023$season, levels = c("Snow Melt", "Growing Season", "Zero curtain", "Winter"))
...
ggplot(seasonal_integrals_2023, aes(x = season, y = integral_value)) +
  ...
  labs(
    x = "Season",
    y = expression("Net CO"[2] * " Flux (Grams of Carbon / m"^2*")"),
...
ggplot(seasonal_integrals_2023, aes(x = season, y = integral_value)) +
  ...
  labs(
    x = "Season",
    y = expression("Net CH"[4] * " Flux (Milligrams of Carbon / m"^2*")"),
...
scale_color_gradient(low = "navy", high = "salmon")+
  labs(
    ...
    y = expression("Half-hourly nighttime CO"[2] * " Flux ("*m*mol~m^-2~s^-1*")"),
...
scale_color_gradient(low = "navy", high = "salmon")+
  labs(
    x = expression("5 cm Soil Temperature ("*degree*"C)"),
    y = expression("Half-hourly CH"[4] * " Flux ("*m*mol~m^-2~s^-1*")"),
...
exponential_fit_plot <- function(data, x_var, y_var) {
  exp_model <- nls(
    formula = as.formula(paste(y_var, "~ a * exp(b *", x_var, ")")),
    data = data,
...
ggplot(data=df_2023)+
  ...
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = 0)+
  geom_line(aes_string(y = fit), color = "black") +
  geom_point(aes(x=TS_1_05_1, y=FC_night, col = DOY))+
scale_color_gradient(low = "navy", high = "salmon")+
  labs(
    x = expression("5cm Soil Temperature ("*degree*"C)"),
