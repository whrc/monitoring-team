...
ggplot(data = df_avg_2023, aes(x=Day))+
  ...
  scale_y_continuous(expression('CO'[2]*' Flux (gC'~m^-2~d^-1*')'))+
  scale_x_datetime(expression(''))+
  scale_color_manual(name = " ", values=c("NEE" = "navy", "-GPP" = "turquoise3", "Respiration"="salmon2"))
...
## change to ameriflux submission to get different value? 11/12 ish?; Resolved, Marco uses gCH4 not gC 
...
ggplot(data = df_avg_2023) +
  ...
  scale_y_continuous(name = expression("NEE ("*gC~m^-2~d^-1*")"), sec.axis = sec_axis(~ . * 15, name = expression("Temperature ("*degree*"C)")))+
  scale_x_datetime(name="")+
  ...
  #end of zero curtain, DOY 295 (10-22)
...
ggplot(seasonal_integrals_2023, aes(x = season, y = integral_value)) +
  ...
  labs(
    x = " ",
    y = expression("Net CO"[2] * " Flux (gC m"^-2*")"),
...
ggplot(seasonal_integrals_2023, aes(x = season, y = integral_value)) +
  ...
  labs(
    x = "",
    y = expression("Net CH"[4] * " Flux (gC m"^-2*")"),
