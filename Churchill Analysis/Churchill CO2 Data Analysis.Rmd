...
```{r, warning=FALSE, echo=FALSE}
df1_F = fread('C:/Users/dtrangmoe/Documents/CA-CF1 FullSet/AMF_CA-CF1_FLUXNET_FULLSET_2007-2008_3-5/AMF_CA-CF1_FLUXNET_FULLSET_HH_2007-2008_3-5.csv', na.strings="-9999")
```
...
df1_F$TIMESTAMP_END = as.character(df1_F$TIMESTAMP_END)
df1_F$TIMESTAMP_START = as.character(df1_F$TIMESTAMP_START)

df1_F$TIMESTAMP_END = as.POSIXct(df1_F$TIMESTAMP_END, tz="UTC", format = "%Y%m%d%H%M")
df1_F$TIMESTAMP_START = as.POSIXct(df1_F$TIMESTAMP_START, tz="UTC", format = "%Y%m%d%H%M")
...
# Set to TRUE to view plots as they're created 
ViewPlots = TRUE
...
# full timeseries
FullSeries <- ggplot(data = subset(df1_F,df1_F$NEE_VUT_REF_QC !=3)) + 
  geom_point(aes(x = TIMESTAMP_END, y = NEE_VUT_REF, color=NEE_VUT_REF_QC ))+
  geom_hline(yintercept = 0)+
  scale_x_datetime(expression('Time'))+
  scale_y_continuous(expression('NEE'))
...
#Spring '07
Spring07 <- ggplot(data = df1_F) + 
  geom_point(aes(x = TIMESTAMP_END, y = NEE_VUT_REF))+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = as.POSIXct('2007-06-19'))+
  scale_x_datetime(expression('Time'), limits=as.POSIXct(c('2007-06-05','2007-06-30')))+
  scale_y_continuous(expression('NEE'),limits=c(-2.5, 1))
...
df1 = filter(df1_F, !(TIMESTAMP_END <= as.POSIXct('2007-06-19')))


#Fall '07
Fall07 <- ggplot(data = df1_F) + 
  geom_point(aes(x = TIMESTAMP_END, y = NEE_VUT_REF))+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = as.POSIXct(c('2007-09-12','2007-10-01','2007-10-12')))+
  scale_x_datetime(expression('Time'),limits=as.POSIXct(c('2007-09-01','2007-10-20')))+
  scale_y_continuous(expression('NEE'),limits=c(-5, 8))

if (ViewPlots == TRUE){
  Fall07
}

df1 = filter(df1, !(TIMESTAMP_END >= as.POSIXct('2007-09-12') & TIMESTAMP_END <= as.POSIXct('2007-10-01')))

#Spring '08
Spring08 <- ggplot(data = df1_F) + 
  geom_point(aes(x = TIMESTAMP_END, y = NEE_VUT_REF))+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = as.POSIXct('2008-06-06'))+
  scale_x_datetime(expression('Time'),limits=as.POSIXct(c('2008-05-25','2008-06-20')))+
  scale_y_continuous(expression('NEE'),limits=c(-5, 8))
...
df1 = filter(df1, !(TIMESTAMP_END >= as.POSIXct('2007-10-12') & TIMESTAMP_END <= as.POSIXct('2008-06-06')))

#Fall '08
Fall08 <- ggplot(data = df1_F) + 
  geom_point(aes(x = TIMESTAMP_END, y = NEE_VUT_REF))+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = as.POSIXct('2008-10-19'))+
  scale_x_datetime(expression('Time'),limits=as.POSIXct(c('2008-10-01','2008-11-01')))+
  scale_y_continuous(expression('NEE'),limits=c(-3, 3))

if (ViewPlots == TRUE){
  Fall08
}

df1 = filter(df1, !(TIMESTAMP_END >= as.POSIXct('2008-10-19') ))

FullSeries2 <- ggplot(data = df1) + 
  geom_point(aes(x = TIMESTAMP_END, y = NEE_VUT_REF))+
  geom_hline(yintercept = 0)+ scale_x_datetime(expression('Time'))+
  scale_y_continuous(expression('NEE'))
...
grid.arrange(FullSeries, FullSeries2, Spring07, Fall07, Spring08, Fall08, nrow=3, ncol=2)

rm(Fall07, Fall08, Spring07, Spring08, FullSeries, FullSeries2, df1_F)
