...
```{r}
#defines seasons by Oechel et al. 2014
#only works for 2023 right now
...
# simpler function but doesn't work

# assign_seasons <- function(df, column_name, spring_threshold = 0, summer_threshold = 100, min_days = 3) {
#   
#   # Helper function to find start date of consecutive days above threshold
#   find_start_date <- function(vec, threshold, min_days) {
#     rle_vec <- rle(vec > threshold)
#     end_indices <- cumsum(rle_vec$lengths)
#     start_indices <- end_indices - rle_vec$lengths + 1
#     for (i in seq_along(rle_vec$values)) {
...
#   }
#   
#   # Find start dates for each season
#   spring_start <- find_start_date(df[[column_name]], spring_threshold, min_days)
#   summer_start <- find_start_date(df[[column_name]], summer_threshold, min_days)
#   
#   # Assume fall_start and winter_start are found
#   fall_start <- find_start_date(df[[column_name]][(summer_start + 1):nrow(df)], summer_threshold, min_days)
#   winter_start <- find_start_date(df[[column_name]][(summer_start + 1):nrow(df)], spring_threshold, min_days)
#   
#   # If fall_start or winter_start is not found, set it to the last day + 1
#   if (is.na(fall_start)) {
#     fall_start <- nrow(df) + 1
#   }
#   if (is.na(winter_start)) {
#     winter_start <- nrow(df) + 1
#   }
#   
#   # Assign seasons based on start dates
#   df <- df %>%
#     mutate(season = case_when(
#       row_number() < spring_start ~ 1,    # Winter
#       row_number() >= spring_start & row_number() < summer_start ~ 2,  # Spring
#       row_number() >= summer_start & row_number() < fall_start ~ 3,    # Summer
#       row_number() >= fall_start & row_number() < winter_start ~ 4,    # Fall
#       row_number() >= winter_start ~ 1    # Winter
#     ))
#   rm(fall_start, winter_start, summer_start, spring_start)
#   return(plot(df$DOY, df$season))
# }
...
To do: add titles to bars describing season
...
ggplot(data = df_avg_2023, aes(x=DOY, y=cumulative))+
  ...
  scale_x_continuous(lim = c(0, 365))+
  geom_hline(yintercept=0, col="black")+
  scale_color_manual(values = c("blue", "green", "orange", "red"), name = "Season", labels = c("1", "2", "3", "4"))+ 
  scale_y_continuous(expression('Cumulative NEE ('*m*mol~m^-2~s^-1*')'))
  
# Add highlights here to show missed data in summer-only analysis
