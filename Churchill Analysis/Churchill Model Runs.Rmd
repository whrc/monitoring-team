...
ggplot(data = CF3) +
  theme_bw() +
  geom_point(aes(x = ts, y = H20), color = 'salmon') + 
  geom_hline(yintercept = 0) +
  scale_y_continuous(
...
df23 <- df %>%
  mutate(
      season = case_when(
      (DOY >= 260 & DOY <= 320) ~ 'zero curtain',
      (DOY >= 124 & DOY <= 166) ~ 'inital thaw',
      (DOY >= 167 & DOY <= 259) ~ 'growing season',
      (DOY >= 321 | DOY <= 123) ~ 'winter freeze',
      TRUE ~ NA_character_
    )
  ) %>%
  filter(year == 2023)
