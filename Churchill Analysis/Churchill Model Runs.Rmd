...
## LM 5 model
```{r}
# Extract coefficients from the TS5logCH4 model
intercept <- coef(TS5logCH4)["(Intercept)"]
slope <- coef(TS5logCH4)["TS_2_05_1"]

TS_2_05_1_seq <- seq(min(na.omit(df$TS_2_05_1)), max(na.omit(df$TS_2_05_1)), length.out = num_rows_df)
df$TS_2_05_1_seq = df$TS_2_05_1_seq

plot(df$ts,df$TS_2_05_1_seq)
model_data <- data.frame(TS_2_05_1 = seq(min(na.omit(df$TS_2_05_1)), max(na.omit(df$TS_2_05_1)), length.out = num_rows_df))


# Extract the predicted log10(FCH4 + 5) values from the model
predicted_log10_FCH4_plus_5 <- predict(TS5logCH4)

# Exponentiate the predicted log10(FCH4 + 5) values to obtain predicted FCH4 values
predicted_FCH4 <- 10^predicted_log10_FCH4_plus_5 - 5
