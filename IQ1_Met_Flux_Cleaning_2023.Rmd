...
plot(df$ts,df$co2_flux, abline(h=40, col= 'red')); abline(h=-40, col= 'red'); abline(h=0,col= 'black')
 

# Clean based on graphs, create new variable, and see cleaned product


co2_flux.limits = replace(df$co2_flux,df$co2_flux > 40 | df$co2_flux < -40,NA)
...
plot(df$ts,df$h2o_flux);abline(h=-2, col= 'red'); abline(h=5, col= 'red'); abline(h=0,col= 'black')
...
plot(df$ts,df$H);abline(h=-60, col= 'red')
...
plot(df$ts,df$LE);abline(h=-80, col= 'red');abline(h=170, col= 'red')
 

# Clean based on graphs, create new variable, and see cleaned product

  
LE.limits = replace(df$LE, df$LE < -80 | df$LE > 170,NA)
...
colnames(df)[which(names(df) == 'Fc_irga')] <- 'co2_flux'

colnames(df)[which(names(df) == 'LE_irga')] <- 'LE'
...
#### R^2^ = `r summary(lm(df$flux_EB ~ df$met_EB))$r.squared`: Really bad H values
...
plot(df$TIMESTAMP,df$Tau); abline(h=-1, col= 'red'); abline(h=1, col= 'red'); abline(h=0,col= 'black')
...
plot(df$TIMESTAMP,df$co2_flux, abline(h=14, col= 'red')); abline(h=-16, col= 'red'); abline(h=0,col= 'black')
 

# Clean based on graphs, create new variable, and see cleaned product


co2_flux.limits = replace(df$co2_flux,df$co2_flux > 14 | df$co2_flux < -16,NA)
hist(co2_flux.limits,breaks=100)
plot(df$TIMESTAMP, co2_flux.limits)
...
H.limits = replace(df$H, df$H < -70 | df$H > 350 | df$TIMESTAMP > as.POSIXct('2023-10-05 200000'),NA)
hist(H.limits,breaks = 100)

# H gets too crazy after 10-05 20:00- remove all data after 
...
ggplot(data = df)+geom_hline(yintercept = 0)+
  ...
  scale_x_datetime(limits = as.POSIXct(c('2023-05-29','2023-10-01'),format="%F"))

ggplot(data = df)+geom_hline(yintercept = 0)+geom_vline(xintercept = as.POSIXct('2023-10-05 200000'))+
  geom_point(aes(x=TIMESTAMP,y=H),col = 'red')+
  geom_point(aes(x=TIMESTAMP,y=H.limits),col = 'black')+
  scale_y_continuous(limits = c(-400,1750))+
  scale_x_datetime(limits = as.POSIXct(c('2023-10-01','2023-10-22'),format="%F"))
...
plot(df$TIMESTAMP,df$LE);abline(h=350, col= 'red');abline(h=-150, col= 'red')
 

# Clean based on graphs, create new variable, and see cleaned product

  
LE.limits = replace(df$LE, df$LE < -150 | df$LE > 350,NA)
hist(LE.limits)
...
ggplot(data = df, aes(x=TIMESTAMP, y=nightCO2))+
  geom_point()+
