...
#use 80% of data set as training set and 10% as test set
sample = sample(c(TRUE, FALSE), nrow(cc), replace=TRUE, prob=c(0.8,0.2))
train  = cc[sample, ]
test   = cc[!sample, ]
...
yk1nee = ggplot(data = test.data,aes(rfnee,co2_flux.c))+theme_bw()+ggtitle('US-YK1 | Burned')+
  ...
  geom_text(aes(7,-8),label=expression(R^2~"= 0.83"))+
  geom_text(aes(7,-10),label=expression(Slope~"= 1.00"))
...
yk1ch4 = ggplot(data = test.data,aes(rfch4,ch4_flux.c))+theme_bw()+ggtitle("US-YK1 | Burned")+
  geom_hline(yintercept = 0,lty=2)+
  geom_vline(xintercept = 0,lty=2)+
  geom_point(alpha=0.2)+
  scale_fill_viridis_c()+
  geom_abline(slope = 1,intercept = 0,col='red',lty=1)+
  scale_x_continuous(limits = c(-0.05,0.1),expression('RF '*CH[4]~flux~" ("*mu*mol~CH[4]~m^-2~s^-1*')'))+
  scale_y_continuous(limits = c(-0.05,0.1),expression('Eddy Covariance '*CH[4]~flux~" ("*mu*mol~CH[4]~m^-2~s^-1*')'))+
  geom_text(aes(0.075,-0.01),label=expression(R^2~"= 0.49"))+
  geom_text(aes(0.075,-0.03),label=expression(Slope~"= 1.14"))
...
plot_grid(yk1nee,yk1ch4,labels = c('a','b'))
