rm(list = ls())
library(REddyProc)
setwd('C:/Users/karndt.WHRC/Desktop/sites/YKD/Ameriflux YK/burned/') #use this to set your directory

#help("REddyProc-package")

  #+++ Load data with one header and one unit row from (tab-delimited) text file
    EddyData.F <- fLoadTXTIntoDataframe('./reddy_ykdb.txt')
...
#+++ with all variables needed for processing later
  EddyProc.C <- sEddyProc$new('YKD Burned', EddyDataWithPosix.F, c('NEE','CH4','H','LE','Ustar','Rg','Tair','Tsoil','RH','VPD'))
  EddyProc.C$sSetLocationInfo(LatDeg =  61.2723, LongDeg = -163.2228, TimeZoneHour = -9)
...
#  EddyProc.C$sPlotHHFluxes('Reco')
  ...
  fWriteDataframeToFile(CombinedData.F, 'ykdb-Results_data.txt', 'data')
